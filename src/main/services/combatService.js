...
import { getLevel, getXpForLevel } from './gameService.js'
 ...
 */
export function getTypeEffectiveness(attackerTypes, defenderTypes) {
  let multiplier = 1
...
export class CombatService {
  ...
  /**
   * Génère un adversaire sauvage.
   */
  generateOpponent(zoneId, playerLevel) {
    const zone = zones.find(z => z.id === zoneId)
    let possibleSpecies = pokedex.map(p => p.id)

    if (zone && zone.pokemon.length > 0) {
  ...
  startCombat(activeId, zoneId) {
    ...
    const combatState = {
      ...
      turn: initialTurn,
      log: [
        `Un ${opponent.label} sauvage apparaît !`,
        `Le combat commence ! (${initialTurn === 'player' ? playerData?.label : opponent.label} est plus rapide)`
      ],
      isFinished: false,
      result: null,
      captured: false
  ...
  executeTurn(combatState) {
    ...
    if (turn === 'player') {
      ...
      newState.log.push(`${player.label} attaque ! ... inflige ${result.damage} dégâts !${effectMsg}`)
      ...
      if (newOpponentHp <= 0) {
        ...
        newState.log.push(`L'ennemi ${opponent.label} est K.O. !`, `Vous avez gagné !`)
      ...
      newState.log.push(`L'ennemi ${opponent.label} attaque ! ... inflige ${result.damage} dégâts !${effectMsg}`)
      newState.playerHp = newPlayerHp

      if (newPlayerHp <= 0) {
        newState.log.push(`Votre Pokémon est K.O...`, `Vous prenez la fuite...`)
        newState.isFinished = true
  ...
  /**
   * Finalise le combat et distribue les récompenses.
   */
